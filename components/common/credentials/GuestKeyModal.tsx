...
import { maskKey, setMemoryCredential, setPersistentCredential, setSessionCredential } from '@/lib/security/web-crypto'
...
type Props = {
  ...
  onSaved: (args: { provider: string; masked: string; scope: StorageScope }) => void
}

export function GuestKeyModal({ open, onOpenChange, defaultProviderId, onSaved }: Props) {
  ...
  const providerOptions = useMemo(
    () => PROVIDERS.filter((p) => ['openai', 'anthropic', 'google', 'mistral', 'perplexity', 'xai', 'openrouter'].includes(p.id)),
    []
  )
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Keys are handled entirely on your device. Guest keys are never stored on our servers.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-1.5">
            ...
            />
            {masked && (
              <div className="text-xs text-muted-foreground">Masked: {masked}</div>
            )}
          </div>

          <div className="grid gap-1.5">
            <Label>Storage Scope</Label>
            <RadioGroup value={scope} onValueChange={(v) => setScope(v as StorageScope)}>
              <div className="flex items-center gap-2">
                <RadioGroupItem value="request" id="scope-request" />
                <Label htmlFor="scope-request">Request-only</Label>
                <span className="text-xs text-muted-foreground">Do not store; send once</span>
              </div>
              <div className="flex items-center gap-2">
                <RadioGroupItem value="tab" id="scope-tab" />
                <Label htmlFor="scope-tab">Tab (memory)</Label>
                <span className="text-xs text-muted-foreground">Ephemeral; cleared on reload</span>
              </div>
              <div className="flex items-center gap-2">
                <RadioGroupItem value="session" id="scope-session" />
                <Label htmlFor="scope-session">Session (encrypted)</Label>
                <span className="text-xs text-muted-foreground">Encrypted and auto-attached</span>
              </div>
              <div className="flex items-center gap-2">
                <RadioGroupItem value="persistent" id="scope-persistent" />
                <Label htmlFor="scope-persistent">Persistent (encrypted with passphrase)</Label>
              </div>
            </RadioGroup>
          </div>
          ...
          {scope === 'persistent' && (
            <div className="grid gap-1.5">
              ...
              <Input type="password" value={passphrase} onChange={(e) => setPassphrase(e.target.value)} placeholder="Required for persistent" />
            </div>
          )}

          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Switch checked={validateFormat} onCheckedChange={setValidateFormat} id="validate-format" />
              <Label htmlFor="validate-format">Validate key format</Label>
            </div>
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="text-xs text-muted-foreground underline cursor-help">Security</span>
              </TooltipTrigger>
              <TooltipContent>
                Keys never leave your device except per-request transmission; we never store guest keys server-side.
              </TooltipContent>
            </Tooltip>
          </div>
