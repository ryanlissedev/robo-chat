...
export class PersonalityManager {
  ...
  triggerSafetyProtocol(
    ...
    const _currentPersonality = this.getCurrentPersonality();
  ...
  requestConfirmation(
    ...
    return new Promise((resolve) => {
      ...
      const requiresConfirmation =
        currentPersonality.features.requiresConfirmation;
      const shouldConfirm =
  ...
  recordSessionEnd(duration: number, userRating?: number): void {
    ...
    if (metrics) {
      ...
      if (userRating !== undefined) {
        metrics.userSatisfactionRating =
          (metrics.userSatisfactionRating * (totalSessions - 1) + userRating) /
          totalSessions;
      }
    ...
    if (metrics) {
      ...
      if (!existing) {
        ...
        if (metrics.mostCommonQuestions.length > 10) {
          metrics.mostCommonQuestions = metrics.mostCommonQuestions.slice(
            0,
            10
          );
        }
  ...
  suggestPersonalitySwitch(context: string): PersonalityMode | null {
    ...
    if (
      ...
      context.toLowerCase().includes('danger')
    ) {
    ...
    if (
      context.toLowerCase().includes('technical') ||
      context.toLowerCase().includes('specification')
    ) {
    ...
    if (
      context.toLowerCase().includes('explain') ||
      context.toLowerCase().includes('help me understand')
    ) {
    ...
    try {
      ...
      if (parsed.metrics) {
        ...
        Object.entries(parsed.metrics).forEach(([mode, metrics]) => {
          this.metrics.set(
            mode as PersonalityMode,
            metrics as PersonalityMetrics
          );
        });
      }
...
export function createPersonalityManager(
  settings?: Partial<VoicePersonalitySettings>
): PersonalityManager {
  return new PersonalityManager(settings);
}

export function getRecommendedPersonality(
  userQuery: string,
  ...
  if (
    safetyKeywords.some((keyword) => query.includes(keyword)) &&
  ...
  if (
    helpKeywords.some((keyword) => query.includes(keyword)) &&
    currentMode !== 'friendly-assistant'
  ) {
    return 'friendly-assistant';
  }
