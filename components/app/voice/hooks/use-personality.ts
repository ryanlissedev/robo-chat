...
import { PersonalityManager, personalityManager } from '../config/personality-manager';
import { PersonalityMode, PersonalityConfig, VoicePersonalitySettings, PersonalityEvent, PersonalityEventType } from '../config/personality-types';
import { getPersonalityConfig, getPersonalityModes } from '../config/personality-configs';
...
export function usePersonality(options: UsePersonalityOptions = {}) {
    ...
    try {
      ...
      if (success) {
        ...
        if (enableEventLogging) {
          console.log(`Personality switched to: ${newConfig.name}`, {
      ...
      const err = error instanceof Error ? error : new Error('Failed to switch personality');
      setState(prev => ({ ...prev, isLoading: false, error: err }));
      return false;
    }
  }, [manager, updateState, sessionId, enableEventLogging, state.currentMode]);

  // Get personality recommendations based on context
  const getPersonalityRecommendation = useCallback((context: string): PersonalityMode | null => {
    return manager.suggestPersonalitySwitch(context);
  }, [manager]);

  // Auto-switch personality based on transcript content
  const handleAutoSwitch = useCallback((transcript: string) => {
    ...
    if (recommendation && recommendation !== state.currentMode) {
      switchPersonality(recommendation, 'Auto-switch based on content analysis');
    }
  }, [enableAutoSwitch, getPersonalityRecommendation, state.currentMode, switchPersonality]);

  // Generate voice instructions
  const generateVoiceInstructions = useCallback((customContext?: string): string => {
  ...
  // Update settings
  const updateSettings = useCallback((updates: Partial<VoicePersonalitySettings>) => {
  ...
  const endPersonalitySession = useCallback((duration: number, userRating?: number) => {
  ...
  const getMetrics = useCallback((mode?: PersonalityMode) => {
  ...
  const getEventHistory = useCallback((eventType?: PersonalityEventType) => {
  ...
  const personalityFeatures = useMemo(() => state.currentConfig.features, [state.currentConfig]);
  const audioSettings = useMemo(() => state.currentConfig.audioSettings, [state.currentConfig]);
  const voiceSettings = useMemo(() => state.currentConfig.voiceSettings, [state.currentConfig]);
  ...
  return {
    ...
    requiresConfirmation: (action: string, riskLevel: 'low' | 'medium' | 'high') => {
    ...
    usesTechnicalTerminology: () => personalityFeatures.usesTechnicalTerminology,
...
export function usePersonalitySwitcher() {
  const { switchPersonality, currentMode, availableModes, isLoading, canSwitchPersonality } = usePersonality();
  return { switchPersonality, currentMode, availableModes, isLoading, canSwitchPersonality };
}
