...
export async function deriveKeyFromPassphrase(
  ...
  return window.crypto.subtle.deriveKey(
    { name: PBKDF2_ALG, hash: 'SHA-256', salt, iterations },
    keyMaterial,
...
export async function importAesKeyRaw(raw: Uint8Array): Promise<CryptoKey> {
  assertBrowserCrypto();
  return window.crypto.subtle.importKey('raw', raw, { name: AES_ALG }, false, [
    'encrypt',
    'decrypt',
  ]);
}
...
const MEM_STORE = new Map<string, EncryptBundle & { masked: string }>();
...
export async function setMemoryCredential(provider: string, plaintextKey: string) {
  ...
  const bundle = await encryptWithKey(plaintextKey, key);
  MEM_STORE.set(provider, { ...bundle, masked: maskKey(plaintextKey) });
  return { masked: maskKey(plaintextKey) };
}

export function getMemoryCredential(provider: string) {
  return MEM_STORE.get(provider) || null;
}
...
export async function getSessionCredential(provider: string): Promise<{ masked: string; plaintext: string } | null> {
  ...
  const bundle = JSON.parse(raw) as EncryptBundle;
