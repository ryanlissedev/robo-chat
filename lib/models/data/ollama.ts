...
function extractFamilyFromName(modelName: string): string {
  ...
  if (name.includes('codegemma')) {
    return 'CodeGemma';
  }
  if (name.includes('starcoder')) {
    return 'StarCoder';
  }
  if (name.includes('wizardcoder')) {
    return 'WizardCoder';
  }
...
function generateTags(
  ...
  if (parameterSize !== 'Unknown') {
    tags.push(parameterSize.toLowerCase());
  }
...
function estimateContextWindow(family: string, modelName: string): number {
  ...
  if (name.includes('32k')) {
    return 32_768;
  }
  if (name.includes('128k')) {
    return 131_072;
  }
  if (name.includes('1m')) {
    return 1_048_576;
  ...
  switch (family) {
    case 'Llama':
      return name.includes('3.2') || name.includes('3.1') ? 128_000 : 4096;
    case 'Qwen':
      return 32_768;
    case 'DeepSeek':
      return 163_840;
...
function estimateIntelligence(
  ...
  const sizeNum = Number.parseFloat(parameterSize.replace(/[^0-9.]/g, ''));
