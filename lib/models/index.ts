import { FREE_MODELS_IDS, GUEST_ALLOWED_MODELS } from '../config';
...
export async function getModelsWithAccessFlags(): Promise<ModelConfig[]> {
  ...
  // Models accessible to guests (includes free models and Ollama)
  const guestAccessibleModels = models
    .filter(
      (model) =>
        GUEST_ALLOWED_MODELS.includes(model.id) || model.providerId === 'ollama'
    )
    .map((model) => ({
      ...model,
      accessible: true,
  ...
  // Pro models that require authentication and API keys
  const proModels = models
    .filter(
      (model) => !guestAccessibleModels.map((m) => m.id).includes(model.id)
    )
    .map((model) => ({
      ...model,
      accessible: false,
  ...
  return [...guestAccessibleModels, ...proModels];
}
