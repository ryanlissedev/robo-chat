...
const MODEL_PROVIDER_MAP: Record<string, Provider> = {
  ...
  // GPT-5 family (custom mapping used in app)
  'gpt-5-mini': 'openai',
  'gpt-5': 'openai',
  'gpt-5-pro': 'openai',
  'gpt-5-nano': 'openai',
  'gpt-5-thinking': 'openai',
  'gpt-5-thinking-mini': 'openai',
...
export function getProviderForModel(model: string): Provider {
  ...
  // For unknown models, try to infer from model name patterns
  if (
    model.includes('gpt') ||
    model.includes('o1') ||
    model.includes('o3') ||
  ...
  // Default to openai for unknown models (most common case)
  return 'openai';
}
