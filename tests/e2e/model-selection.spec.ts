...
import { waitForPageReady, mockApiResponse } from './fixtures'
...
test.describe('Model Selection', () => {
  ...
  test.describe('When opening model selector', () => {
    test('should display available models', async ({ modelSelector, page }) => {
      ...
      for (const model of mockModels) {
        await expect(page.locator(`[data-testid="model-option-${model.id}"]`)).toBeVisible()
        await expect(page.locator(`text=${model.name}`)).toBeVisible()
      }
    })

    test('should show model details and pricing', async ({ modelSelector, page }) => {
      await modelSelector.openModelSelector()
    ...
    test('should highlight favorite models', async ({ modelSelector, page }) => {
      await modelSelector.openModelSelector()

      // Favorite models should have special styling or indicators
      await expect(page.locator(`[data-testid="model-option-gpt-4"]`)).toHaveAttribute('data-favorite', 'true')
      await expect(page.locator(`[data-testid="model-option-claude-3-sonnet"]`)).toHaveAttribute('data-favorite', 'true')
      await expect(page.locator(`[data-testid="model-option-gpt-3.5-turbo"]`)).not.toHaveAttribute('data-favorite', 'true')
    })
    ...
    test('should filter models by search', async ({ modelSelector, page }) => {
      ...
      if (await searchInput.isVisible()) {
        ...
        await expect(page.locator(`[data-testid="model-option-gpt-3.5-turbo"]`)).toBeVisible()
        await expect(page.locator(`[data-testid="model-option-claude-3-sonnet"]`)).not.toBeVisible()
    ...
    test('should close selector with escape key', async ({ modelSelector, page }) => {
      await modelSelector.openModelSelector()

      // Verify selector is open
      await expect(page.locator('[data-testid="model-selector-content"]')).toBeVisible()

      // Close with escape
      await modelSelector.closeModelSelector()

      // Verify selector is closed
      await expect(page.locator('[data-testid="model-selector-content"]')).not.toBeVisible()
    })
  })

  test.describe('When selecting a model', () => {
    test('should update selected model display', async ({ modelSelector, page }) => {
    ...
    test('should persist model selection across page navigation', async ({ modelSelector, page }) => {
      // Select a model
      await modelSelector.openModelSelector()
    ...
    test('should send messages with selected model', async ({ modelSelector, chatPage, page }) => {
      ...
      await page.route('**/api/chat', async route => {
        ...
        const stream = new ReadableStream({
          start(controller) {
            controller.enqueue(encoder.encode('data: {"content":"Response from Claude 3 Sonnet"}\n\n'))
            controller.close()
          }
        })
    ...
    test('should show model-specific features when available', async ({ modelSelector, page }) => {
      await modelSelector.openModelSelector()
      ...
      const modelCapabilities = page.locator('[data-testid="model-capabilities"]')
  ...
  test.describe('When managing favorite models', () => {
    test('should add model to favorites', async ({ modelSelector, page }) => {
      ...
      const favoriteButton = page.locator(`[data-testid="favorite-button-gpt-3.5-turbo"]`)
      
      if (await favoriteButton.isVisible()) {
        // Mock the API call to update favorites
        await page.route('**/api/user-preferences/favorite-models', async route => {
          await route.fulfill({
            status: 200,
        ...
        await expect(page.locator(`[data-testid="model-option-gpt-3.5-turbo"]`)).toHaveAttribute('data-favorite', 'true')
      }
    })

    test('should remove model from favorites', async ({ modelSelector, page }) => {
      await modelSelector.openModelSelector()

      // Find a favorite model
      const favoriteButton = page.locator(`[data-testid="favorite-button-gpt-4"]`)
      ...
      if (await favoriteButton.isVisible()) {
        ...
        // Should update the favorite status
        await expect(page.locator(`[data-testid="model-option-gpt-4"]`)).not.toHaveAttribute('data-favorite', 'true')
      }
    })

    test('should show favorites section at top', async ({ modelSelector, page }) => {
      await modelSelector.openModelSelector()
  ...
  test.describe('When handling model availability', () => {
    test('should disable unavailable models', async ({ page }) => {
      ...
      const { result: modelSelectorResult } = await page.evaluate(() => {
        return new Promise(resolve => {
          const button = document.querySelector('[data-testid="model-selector-trigger"]') as HTMLElement
      ...
      await expect(page.locator(`[data-testid="model-option-claude-3-sonnet"]`)).toHaveAttribute('data-disabled', 'true')
      
      // OpenAI models should be enabled
      await expect(page.locator(`[data-testid="model-option-gpt-4"]`)).not.toHaveAttribute('data-disabled', 'true')
    })

    test('should show API key setup prompt for disabled models', async ({ modelSelector, page }) => {
      // Mock limited API key access
      await mockApiResponse(page, '/api/user-key-status', {
        openai: false,
        anthropic: false,
    ...
    test('should handle model loading errors gracefully', async ({ modelSelector, page }) => {
      // Mock API failure
      ...
      await expect(page.locator('[data-testid="models-error-state"]')).toBeVisible()
    })
  })

  test.describe('When using keyboard navigation', () => {
    test('should navigate models with arrow keys', async ({ modelSelector, page }) => {
      await modelSelector.openModelSelector()
      ...
      await expect(page.locator(`[data-testid="model-option-gpt-4"]`)).toHaveClass(/highlighted|focused/)

      // Navigate to next model
      await page.keyboard.press('ArrowDown')
      
      // Should highlight second model
      await expect(page.locator(`[data-testid="model-option-claude-3-sonnet"]`)).toHaveClass(/highlighted|focused/)
    })

    test('should select model with enter key', async ({ modelSelector, page }) => {
      await modelSelector.openModelSelector()
