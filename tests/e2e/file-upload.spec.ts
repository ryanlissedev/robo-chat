...
test.describe('File Upload', () => {
  ...
  const createTestFile = async (page: any, fileName: string, content: string) => {
    return await page.evaluate(({ fileName, content }) => {
  ...
  test.describe('When uploading files', () => {
    test('should show file upload button when available', async ({ chatPage, page }) => {
    ...
    test('should upload a text file successfully', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
    ...
    test('should handle multiple file uploads', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
      ...
      // Wait for first file
      await expect(page.locator('[data-testid="uploaded-file"]').first()).toBeVisible()
    ...
    test('should show file preview and details', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await page.evaluate(() => {
        ...
        Object.defineProperty(file, 'size', { value: 51200 })
        dataTransfer.items.add(file)
        fileInput.files = dataTransfer.files
        fileInput.dispatchEvent(new Event('change', { bubbles: true }))
      })
    ...
    test('should remove uploaded files', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
  ...
  test.describe('When handling upload errors', () => {
    test('should show error for file too large', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
    ...
    test('should show error for unsupported file type', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await page.evaluate(() => {
        const fileInput = document.querySelector('[data-testid="file-input"]') as HTMLInputElement
        const dataTransfer = new DataTransfer()
        const unsupportedFile = new File(['binary data'], 'app.exe', { type: 'application/x-msdownload' })
    ...
    test('should handle network errors gracefully', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await expect(page.locator('[data-testid="file-upload-error"], [data-testid="network-error"]')).toBeVisible()
    })
  })

  test.describe('When sending messages with files', () => {
    test('should include uploaded files in chat messages', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await page.route('**/api/chat', async route => {
        ...
        const stream = new ReadableStream({
          start(controller) {
            controller.enqueue(encoder.encode('data: {"content":"I can see you uploaded a document. Let me analyze it for you."}\\n\\n'))
            controller.close()
          }
        })
    ...
    test('should clear files after successful submission', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await page.route('**/api/chat', async route => {
        ...
        const stream = new ReadableStream({
          start(controller) {
            controller.enqueue(encoder.encode('data: {"content":"Message received with file."}\\n\\n'))
            controller.close()
          }
        })
  ...
  test.describe('When handling different file types', () => {
    test('should handle image files', async ({ fileUpload, chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await page.evaluate(() => {
        ...
        const imageFile = new File(['PNG image data'], 'screenshot.png', { type: 'image/png' })
        Object.defineProperty(imageFile, 'size', { value: 204800 })
      ...
      if (await thumbnail.isVisible()) {
        await expect(thumbnail).toHaveAttribute('src', '/api/files/image-123/thumbnail')
      }
    })
    ...
    test('should handle PDF files', async ({ fileUpload, chatPage, page }) => {
      ...
      await page.evaluate(() => {
        ...
        Object.defineProperty(pdfFile, 'size', { value: 1048576 })
  ...
  test.describe('When using drag and drop', () => {
    test('should handle drag and drop file upload', async ({ chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await page.evaluate(() => {
        const dropZone = document.querySelector('[data-testid="chat-input"]') as HTMLElement
    ...
    test('should show drag overlay during drag operations', async ({ chatPage, page }) => {
      await chatPage.chatInput()
