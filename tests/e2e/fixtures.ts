...
export interface ChatPageFixture {
...
export interface ModelSelectorFixture {
...
export interface FileUploadFixture {
...
export const test = base.extend<{
  ...
  chatPage: async ({ page }, use) => {
    const chatPage: ChatPageFixture = {
      ...
      waitForResponse: async () => {
        ...
        await page.waitForSelector('[data-testid="message-loading"]', { state: 'visible' })
          .catch(() => {}) // Might already be present
        await page.waitForSelector('[data-testid="message-loading"]', { state: 'hidden' })
      ...
      selectModel: async (modelName: string) => {
        ...
        await page.waitForSelector('[data-testid="model-selector-content"]', { state: 'visible' })
  ...
  modelSelector: async ({ page }, use) => {
    const modelSelector: ModelSelectorFixture = {
      ...
      selectModel: async (modelName: string) => {
        await page.click(`[data-testid="model-option-${modelName}"]`)
        await page.waitForSelector('[data-testid="model-selector-content"]', { state: 'hidden' })
      },
  ...
  fileUpload: async ({ page }, use) => {
    const fileUpload: FileUploadFixture = {
      ...
      uploadFile: async (filePath: string) => {
        ...
        await page.waitForSelector('[data-testid="uploaded-file"]', { state: 'visible' })
      },
