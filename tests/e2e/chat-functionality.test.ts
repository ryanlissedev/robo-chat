...
test.describe('Chat Functionality', () => {
  ...
  test('should send a message and receive response', async ({ page }) => {
    ...
    const assistantResponse = page.locator('[role="status"], .assistant-message, [data-role="assistant"]');
  ...
  test('should not show verbosity controls', async ({ page }) => {
    ...
    const verbositySelector = page.locator('[aria-label*="verbosity"], [data-testid*="verbosity"]');
    await expect(verbositySelector).not.toBeVisible();
    
    // Reasoning summary controls should also be removed
    const summarySelector = page.locator('[aria-label*="summary"], [data-testid*="reasoning-summary"]');
    await expect(summarySelector).not.toBeVisible();
  });

  test('should handle model selection', async ({ page }) => {
    // Look for model selector
    const modelSelector = page.locator('button:has-text("GPT"), button:has-text("Claude"), [role="combobox"]').first();
    
    if (await modelSelector.isVisible()) {
      await modelSelector.click();
      
      // Should show model options
      const modelOption = page.locator('[role="option"], [role="menuitem"]').first();
  ...
  test('should handle file uploads', async ({ page }) => {
    ...
    const fileUploadButton = page.locator('button[aria-label*="file"], button[aria-label*="upload"], button:has(svg[class*="paperclip"])');
  ...
  test('should display messages correctly with AI SDK v5 format', async ({ page }) => {
    ...
    await expect(page.locator('text=I can definitely help you with that!')).toBeVisible({ timeout: 5000 });
  });
