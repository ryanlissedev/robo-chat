...
import { test, mockApiResponse, interceptNetworkRequests, takeScreenshotOnFailure } from './fixtures';
...
test.describe('Voice Workflow E2E Tests', () => {
  ...
  test('should complete full voice workflow - record, transcribe, edit, index, and send to chat', async ({ 
    ...
    await page.waitForSelector('[data-testid="voice-button"][data-recording="true"]', {
      timeout: 5000,
    });
    ...
    await page.waitForSelector('[data-testid="voice-button"][data-recording="false"]', {
      timeout: 5000,
    });
    ...
    await textarea.fill('Edited transcript: Hello, this is my voice message for testing.');
    ...
    await page.waitForSelector('[data-testid="indexing-status"][data-status="completed"]', {
      timeout: 10000,
    });
    
    // Verify indexing success message
    await expect(page.locator('[data-testid="indexing-message"]')).toContainText(
    ...
    await expect(chatInput).toHaveValue('Edited transcript: Hello, this is my voice message for testing.');
    
    // Step 7: Send the message to AI
    await chatPage.sendMessage('Edited transcript: Hello, this is my voice message for testing.');
    ...
    const transcriptRequest = requests.find(req => 
  ...
  test('should handle voice recording errors gracefully', async ({ page, chatPage }, testInfo) => {
    await chatPage.chatInput();
  ...
  test('should handle transcript indexing failures', async ({ page, chatPage }, testInfo) => {
    await chatPage.chatInput();
    ...
    await page.waitForSelector('[data-testid="indexing-status"][data-status="failed"]', {
      timeout: 10000,
    });
  ...
  test('should maintain voice session state across component interactions', async ({ 
    ...
    await expect(page.locator('[data-testid="voice-session-status"]')).toHaveText('Recording');
    
    // Check if personality selector is visible and working
    await page.click('[data-testid="personality-selector"]');
    await page.waitForSelector('[data-testid="personality-options"]', { state: 'visible' });
    
    // Select different personality
    await page.click('[data-testid="personality-option-technical-expert"]');
    
    // Verify personality changed
    await expect(page.locator('[data-testid="selected-personality"]')).toContainText(
      'Technical Expert'
    );
    ...
    await expect(page.locator('[data-testid="session-personality"]')).toContainText(
      'Technical Expert'
    );
  ...
  test('should support voice transcript search functionality', async ({ 
      ...
      if (url.includes('query=')) {
        await route.fulfill({
          ...
          body: JSON.stringify({
            ...
            message: 'Vector store found. Use OpenAI Assistants API for semantic search.',
          }),
        });
      }
    });
    ...
    await page.waitForSelector('[data-testid="voice-search-panel"]', { state: 'visible' });
    ...
    await expect(page.locator('[data-testid="search-results-count"]')).toContainText('5 files');
    await expect(page.locator('[data-testid="search-vector-store"]')).toContainText('vs-search-test');
  ...
  test('should integrate voice workflow with multi-model chat', async ({ 
    ...
    await page.evaluate(() => {
      window.postMessage({
        type: 'voice-transcript',
        data: {
          transcript: 'Please analyze this data using advanced reasoning capabilities',
    ...
    // Send the message
    await chatPage.sendMessage('Please analyze this data using advanced reasoning capabilities');
    await chatPage.waitForResponse();
