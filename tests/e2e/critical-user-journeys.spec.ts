...
test.describe('Critical User Journeys', () => {
  ...
  test.describe('New User Onboarding Journey', () => {
    test('should guide new user through first chat experience', async ({
      page,
    }) => {
    ...
    test('should handle unauthenticated user limitations gracefully', async ({
      page,
    }) => {
      ...
      await expect(
        page
          .locator('[data-testid="guest-indicator"]')
          .or(page.locator('text=sign in for more features'))
      ).toBeVisible();
    });
  ...
  test.describe('Authenticated User Journey', () => {
    test('should complete full authenticated user workflow', async ({
      page,
    }) => {
      ...
      await page.click('[data-testid="model-selector"]');
      await expect(
        page.locator('[data-testid="model-claude-3-opus"]')
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="reasoning-panel"]')
      ).toBeVisible();
    });
    ...
    test('should handle API key configuration journey', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="key-status-success"]')
      ).toBeVisible();
  ...
  test.describe('Chat Interaction Journey', () => {
    test('should complete full conversation cycle', async ({ page }) => {
      ...
      await expect(page.locator('[role="assistant"]:nth-child(2)')).toBeVisible(
        { timeout: 30000 }
      );
    ...
    test('should handle file upload workflow', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="message-with-file"]')
      ).toBeVisible();
    });
    ...
    test('should handle reasoning display workflow', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="reasoning-content"]')
      ).toBeVisible();
  ...
  test.describe('Multi-Model Comparison Journey', () => {
    test('should enable and use multi-model comparison', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="multi-model-enabled"]')
      ).toBeChecked();
      ...
      await expect(
        page.locator('[data-testid="multi-chat-container"]')
      ).toBeVisible();
      ...
      await expect(
        page.locator('[data-testid="model-response-gpt-4"]')
      ).toBeVisible({ timeout: 45000 });
      await expect(
        page.locator('[data-testid="model-response-claude-3"]')
      ).toBeVisible({ timeout: 45000 });
    });

    test('should handle individual model selection in multi-mode', async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="response-gpt-4o-mini"]')
      ).toBeVisible({ timeout: 30000 });
      await expect(
        page.locator('[data-testid="response-claude-3-haiku"]')
      ).toBeVisible({ timeout: 30000 });
    });
  });
  ...
  test.describe('Settings and Personalization Journey', () => {
    test('should complete settings configuration journey', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="favorite-models"]')
      ).toContainText('gpt-4');
  ...
  test.describe('Error Recovery Journey', () => {
    ...
    test('should handle API key errors', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="fix-api-key-link"]')
      ).toBeVisible();
    ...
    test('should handle quota exceeded gracefully', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="upgrade-suggestion"]')
      ).toBeVisible();
    });
  });
  ...
  test.describe('Performance and Accessibility Journey', () => {
    ...
    test('should meet accessibility standards', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="send-button"]')).toHaveAttribute(
        'aria-label'
      );
