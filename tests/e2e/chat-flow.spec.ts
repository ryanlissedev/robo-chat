...
test.describe('Chat Message Flow', () => {
  ...
  test.describe('When user sends a message', () => {
    test('should display the message and show loading indicator', async ({ chatPage, page }) => {
      ...
      await page.route('**/api/chat', async route => {
        ...
        const stream = new ReadableStream({
          start(controller) {
            controller.enqueue(encoder.encode('data: {"content":"Hello! I\'d be happy to help you with coding. What specific topic are you working on?"}\n\n'))
            controller.close()
          }
        })
    ...
    test('should handle empty messages gracefully', async ({ chatPage, page }) => {
      await chatPage.chatInput()
    ...
    test('should show error state when API fails', async ({ chatPage, page }) => {
      await chatPage.chatInput()
    ...
    test('should send suggestion as message', async ({ chatPage, page }) => {
      ...
      if (await suggestionButton.isVisible()) {
        ...
        await page.route('**/api/chat', async route => {
          ...
          const stream = new ReadableStream({
            start(controller) {
              controller.enqueue(encoder.encode('data: {"content":"Great suggestion! Let me help you with that."}\n\n'))
              controller.close()
            }
          })
  ...
  test.describe('When managing chat sessions', () => {
    test('should start new chat when no messages exist', async ({ chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await page.route('**/api/chat', async route => {
        ...
        const stream = new ReadableStream({
          start(controller) {
            controller.enqueue(encoder.encode('data: {"content":"Hello! How can I help you today?"}\n\n'))
            controller.close()
          }
        })
    ...
    test('should clear chat when clear button is clicked', async ({ chatPage, page }) => {
      await chatPage.chatInput()
  ...
  test.describe('When handling long conversations', () => {
    test('should maintain scroll position during typing', async ({ chatPage, page }) => {
      await chatPage.chatInput()
      ...
      for (let i = 1; i <= 5; i++) {
        await page.route('**/api/chat', async route => {
          ...
          const stream = new ReadableStream({
            start(controller) {
              controller.enqueue(encoder.encode(`data: {"content":"Response ${i}"}\n\n`))
              controller.close()
            }
          })
    ...
    test('should handle rapid message sending', async ({ chatPage, page }) => {
      ...
      await page.route('**/api/chat', async route => {
        ...
        const stream = new ReadableStream({
          start(controller) {
            controller.enqueue(encoder.encode(`data: {"content":"Response ${responseCount}"}\n\n`))
            controller.close()
          }
        })
  ...
  test.describe('When handling edge cases', () => {
    test('should handle network disconnection gracefully', async ({ chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await expect(page.locator('[data-testid="error-message"], [data-testid="network-error"]')).toBeVisible()
    })
    ...
    test('should handle very long messages', async ({ chatPage, page }) => {
      ...
      await page.route('**/api/chat', async route => {
        ...
        const stream = new ReadableStream({
          start(controller) {
            controller.enqueue(encoder.encode('data: {"content":"I received your long message."}\n\n'))
            controller.close()
          }
        })
    ...
    test('should preserve message across page reload', async ({ chatPage, page }) => {
      await chatPage.chatInput()
      ...
      await page.route('**/api/chat', async route => {
        ...
        const stream = new ReadableStream({
          start(controller) {
            controller.enqueue(encoder.encode('data: {"content":"Persistent response"}\n\n'))
            controller.close()
          }
        })
