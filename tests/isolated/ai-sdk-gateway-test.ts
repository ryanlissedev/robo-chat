...
class AISDKGatewayTester {
  ...
  async testGatewayClass() {
    ...
    try {
      ...
      const result = await gateway.testAISDKGateway(
        'Say "Gateway class works!" in exactly 3 words.'
      );
  ...
  async testFallbackComparison() {
    ...
    if (aiSDKResult && directResult) {
      ...
      if (aiSDKResult === directResult) {
        ...
        log(
          'ℹ️  Different responses (expected due to temperature/randomness)',
          'yellow'
        );
      }
    }
  ...
  async testGatewayStatus() {
    ...
    try {
      ...
      log(
        `   Gateway Configured: ${status.gateway.configured}`,
        status.gateway.configured ? 'green' : 'red'
      );
      log(
        `   AI SDK Configured: ${status.gateway.aiSDKConfigured}`,
        status.gateway.aiSDKConfigured ? 'green' : 'red'
      );
      log(
        `   AI SDK Gateway Working: ${status.aiSDK?.gateway}`,
        status.aiSDK?.gateway ? 'green' : 'red'
      );
      log(
        `   OpenAI Direct: ${status.openai.configured}`,
  ...
  async runAllTests() {
    ...
    if (results.directAISDK && results.gatewayClass) {
      ...
      log(
        '✅ The AI SDK is properly routing through the Vercel AI Gateway',
        'green'
      );
