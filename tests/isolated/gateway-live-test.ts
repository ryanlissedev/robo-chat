  ...
  async testGatewayOpenAI() {
    return this.runTest('Gateway OpenAI API Call', async () => {
      ...
      const response = await gatewayClient.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'user', content: 'Say "gateway works" in exactly 2 words.' },
        ],
        max_tokens: 10,
        temperature: 0,
      });
  ...
  async testAutoMode() {
    return this.runTest('Auto Mode (Gateway with Fallback)', async () => {
      ...
      const response = await apiClient.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'user', content: 'Say "auto mode" in exactly 2 words.' },
        ],
        max_tokens: 10,
        temperature: 0,
      });
  ...
  async testRawGatewayAPI() {
    return this.runTest('Raw Gateway API Call', async () => {
      ...
      const response = await fetch(
        `${process.env.AI_GATEWAY_BASE_URL}/openai/chat/completions`,
        {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            model: 'gpt-4o-mini',
            messages: [
              { role: 'user', content: 'Say "raw API" in exactly 2 words.' },
            ],
            max_tokens: 10,
            temperature: 0,
          }),
