  ...
  for (const endpoint of endpoints) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        log(
          `   ✅ SUCCESS! Response: ${JSON.stringify(data).substring(0, 200)}...`,
          'green'
        );
        return endpoint;
      } else {
...
async function main() {
  ...
  if (!responsesEndpoint && chatWorksWithGPT5) {
    log(
      '🔧 Use standard Chat API instead of Responses API for gateway',
      'blue'
    );
    log(
      '🔧 The gateway handles GPT-5 parameter conversion automatically',
      'blue'
    );
