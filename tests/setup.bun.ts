...
// DOMTokenList is not directly exposed by happy-dom; skip binding in tests
...
// Set up requestAnimationFrame and other timing functions with numeric IDs
const __rafStore = new Map<number, NodeJS.Timeout>();
let __rafIdCounter = 0;

globalThis.requestAnimationFrame = ((
  callback: FrameRequestCallback
): number => {
  const id = ++__rafIdCounter;
  const handle = setTimeout(() => {
    __rafStore.delete(id);
    callback(Date.now());
  }, 16);
  __rafStore.set(id, handle);
  return id;
}) as unknown as typeof globalThis.requestAnimationFrame;

globalThis.cancelAnimationFrame = ((id: number) => {
  const handle = __rafStore.get(id);
  if (handle) {
    clearTimeout(handle);
    __rafStore.delete(id);
  }
