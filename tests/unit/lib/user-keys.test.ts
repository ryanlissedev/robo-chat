...
describe('User Keys Service', () => {
  beforeEach(() => {
    ...
    // Mock environment variables needed by createClient
    process.env.NEXT_PUBLIC_SUPABASE_URL = 'https://test.supabase.co';
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY = 'test-anon-key';

    // Ensure isSupabaseEnabled returns true
    vi.mocked(isSupabaseEnabled).mockReturnValue(true);

    // Mock cookies for Next.js headers - include setAll method
    ...
    // Setup method chaining for Supabase queries - each method returns the same object for chaining
    ...
    // Setup default Supabase client mock to return the client object (not null)
    vi.mocked(createClient).mockResolvedValue(mockSupabaseClient as any);

    // Setup default successful decryption
    vi.mocked(decryptKey).mockReturnValue('mock-decrypted-key');
  });
