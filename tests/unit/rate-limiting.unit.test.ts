...
describe('Rate Limiting Unit Tests', () => {
  ...
  beforeEach(() => {
    // Note: Bun test doesn't have built-in timer mocking like Jest
    // For now, we'll use real timers and adjust tests accordingly
  });
  ...
  describe('Rate Limit Calculations', () => {
    ...
    function calculateRateLimit(
      ...
      const remainingRequests = Math.max(
        0,
        maxRequests - effectiveRequests - (isAllowed ? 1 : 0)
      );
    ...
    test('should reset window when expired', () => {
      ...
      const windowStart = Date.now() - 70_000; // Started 70 seconds ago (expired)
    ...
    test('should calculate different limits for different time windows', () => {
      ...
      const testCases = [
        { maxRequests: 60, windowDuration: 60_000, expected: { perSecond: 1 } }, // 1/second
        {
          maxRequests: 1000,
          windowDuration: 3_600_000,
          expected: { perMinute: 16.67 },
        }, // ~16.67/minute
  ...
  describe('Sliding Window Rate Limiting', () => {
    ...
    function slidingWindowRateLimit(
      ...
      const remainingRequests = Math.max(
        0,
        maxRequests - recentRequests.length - (isAllowed ? 1 : 0)
      );

      const oldestRequestTime =
        recentRequests.length > 0 ? Math.min(...recentRequests) : null;
      const nextResetTime = oldestRequestTime
        ? oldestRequestTime + windowDuration
        : null;
    ...
    test('should allow requests in sliding window', () => {
      ...
      const window: SlidingWindow = {
        timestamps: [
          currentTime - 30_000, // 30 seconds ago
          currentTime - 15_000, // 15 seconds ago
          currentTime - 5000, // 5 seconds ago
        ],
        maxRequests: 10,
        windowDuration: 60_000, // 1 minute window
      };
      ...
      expect(result.oldestRequestTime).toBe(currentTime - 30_000);
    });
    ...
    test('should deny requests when sliding window full', () => {
      ...
      const window: SlidingWindow = {
        timestamps: Array.from(
          { length: 10 },
          (_, i) => currentTime - i * 5000
        ), // 10 requests in last 45 seconds
        maxRequests: 10,
    ...
    test('should exclude expired requests from sliding window', () => {
      ...
      const window: SlidingWindow = {
        timestamps: [
          currentTime - 120_000, // 2 minutes ago (expired)
          currentTime - 90_000, // 1.5 minutes ago (expired)
          currentTime - 30_000, // 30 seconds ago (valid)
          currentTime - 10_000, // 10 seconds ago (valid)
        ],
        maxRequests: 5,
        windowDuration: 60_000, // 1 minute window
      };
    ...
    test('should calculate next reset time correctly', () => {
      ...
      const window: SlidingWindow = {
        timestamps: [
          currentTime - 50_000, // This will expire in 10 seconds
          currentTime - 30_000, // This will expire in 30 seconds
          currentTime - 10_000, // This will expire in 50 seconds
        ],
        maxRequests: 3,
        windowDuration: 60_000,
      };
      ...
      expect(result.isAllowed).toBe(false);
      expect(result.nextResetTime).toBe(currentTime - 50_000 + 60_000);
    });
  });
  ...
  describe('User-Specific Rate Limiting', () => {
    ...
    function calculateUserRateLimit(
      ...
      const newResetTime = isWindowExpired
    ...
    test('should apply different limits for free and pro users', () => {
      ...
      const freeResult = calculateUserRateLimit(
        freeUserLimit,
        60_000,
        currentTime
      );
      const proResult = calculateUserRateLimit(
        proUserLimit,
        60_000,
        currentTime
      );
    ...
    test('should reset user limits when window expires', () => {
      ...
      const expiredUserLimit: UserRateLimit = {
        ...
        windowStart: Date.now() - 120_000, // 2 minutes ago (expired)
        isProUser: false,
      };
  ...
  describe('Global Rate Limiting', () => {
    ...
    function calculateGlobalRateLimit(
      ...
      const remainingRequests = Math.max(
        0,
        maxGlobalRequests - effectiveRequests - (isAllowed ? 1 : 0)
      );
      const requestsPerUser =
        activeUsers > 0 ? Math.floor(effectiveRequests / activeUsers) : 0;
    ...
    test('should enforce global rate limits', () => {
      ...
      const result = calculateGlobalRateLimit(globalLimit, 10_000);
  ...
  describe('Rate Limit Headers and Responses', () => {
    ...
    test('should generate correct rate limit headers when allowed', () => {
      ...
      const resetTime = Date.now() + 30_000; // 30 seconds from now
    ...
    test('should include retry-after header when rate limited', () => {
      ...
      const currentTime = Date.now();
      const resetTime = currentTime + 45_000; // 45 seconds from now
  ...
  describe('Rate Limit Bursting', () => {
    ...
    test('should calculate correct refill timing', () => {
      ...
      expect(result.isAllowed).toBe(false);
      expect(result.nextRefillTime).toBe(Date.now() + 10_000); // 10 seconds later
    });
  });
});
