...
const { mockUseBreakpoint, mockUseKeyShortcut, mockUseModel, mockUseUserPreferences, mockFilterAndSortModels } = vi.hoisted(() => ({
  mockUseBreakpoint: vi.fn(() => false), // Default to desktop
  mockUseKeyShortcut: vi.fn(),
  mockUseModel: vi.fn(),
  mockUseUserPreferences: vi.fn(() => ({
    isModelHidden: vi.fn(() => false),
  })),
  mockFilterAndSortModels: vi.fn((models) => models),
}));
...
vi.mock('@/lib/model-store/utils', () => ({
  filterAndSortModels: mockFilterAndSortModels,
}));
...
vi.mock('@/components/ui/drawer', () => ({
  Drawer: ({ children, open, onOpenChange }: any) => 
    open ? <div data-testid="drawer" role="dialog">{children}</div> : null,
  ...
  DrawerTrigger: ({ children, asChild }: any) => asChild ? children : <div>{children}</div>,
}));
...
vi.mock('@/components/ui/dropdown-menu', () => ({
  ...
  DropdownMenuItem: ({ children, onSelect, ...props }: any) => {
    ...
    return (
      <div {...domProps} onClick={onSelect} role="menuitem">{children}</div>
    );
  },
...
vi.mock('@/components/common/model-selector/pro-dialog', () => ({
  ProModelDialog: ({ isOpen }: any) => isOpen ? <div data-testid="pro-dialog">Pro required</div> : null,
}));
...
describe('MultiModelSelector', () => {
  ...
  beforeEach(() => {
    ...
    mockUseBreakpoint.mockReturnValue(false); // Desktop by default
  });
  ...
  describe('Rendering', () => {
    ...
    it('should render multiple selected models count', () => {
      ...
      expect(screen.getByText((content, element) => {
        return content.includes('3') && content.toLowerCase().includes('models');
      })).toBeInTheDocument();
  ...
  describe('Responsive Behavior', () => {
    it('should render drawer on mobile', async () => {
      ...
      const trigger = screen.queryByText('GPT-4') || screen.queryByRole('button', { hidden: true });
      
      if (trigger) {
        await user.click(trigger);
        // Look for drawer/dialog content
        expect(screen.getByRole('dialog') || screen.getByText(/models/i)).toBeInTheDocument();
      } else {
    ...
    it('should render dropdown on desktop', () => {
      mockUseBreakpoint.mockReturnValue(false); // Desktop
  ...
  describe('Model Selection', () => {
    it('should select a new model', async () => {
      ...
      expect(mockSetSelectedModelIds).toHaveBeenCalledWith(['gpt-4', 'gpt-3.5']);
    });
    ...
    it('should show limit indicator for unselected models when at limit', async () => {
      ...
      const limitIndicator = screen.queryByText(/limit|max/i) || screen.queryByText('Limit');
      expect(limitIndicator || screen.getByText(/2.*2|full/i)).toBeInTheDocument();
    });
  ...
  describe('Accessibility', () => {
    ...
    it('should disable checkboxes appropriately', async () => {
      ...
      const lockedModel = menuItems.find(item => item.textContent?.includes('Locked'));
      const limitModel = menuItems.find(item => item.textContent?.includes('Gemini'));
  ...
  describe('Model Counter and Limits', () => {
    it('should show correct count in drawer title', async () => {
      ...
      try {
        ...
        try {
          const trigger = screen.queryByText('GPT-4') || 
                         screen.queryByText(/models/) ||
                         document.querySelector('[data-testid*="trigger"], [aria-haspopup], [role="combobox"]');
    ...
    it('should show tooltip with shortcut and count', () => {
      ...
      expect(screen.getByTestId('tooltip-content')).toHaveTextContent('Select models ⌘⇧M (1/3)');
    });
  });
  ...
  describe('Model Icons and Providers', () => {
    ...
    it('should handle models without provider icons', async () => {
      const modelsWithoutIcon = [
        { id: 'unknown', name: 'Unknown Model', icon: 'nonexistent', accessible: true }
      ];
  ...
  describe('Empty States', () => {
    ...
    it('should have correct link for model requests', async () => {
      ...
      expect(link).toHaveAttribute('href', 'https://github.com/ibelick/zola/issues/new?title=Model%20Request%3A%20');
