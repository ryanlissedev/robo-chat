...
describe('DialogAuth', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockLocation.href = '';
    (supabaseConfig.isSupabaseEnabled as any).mockReturnValue(true);
  ...
  describe('Rendering', () => {
    ...
    it('should apply correct CSS classes', () => {
      ...
      expect(screen.getByTestId('dialog-description')).toHaveClass('pt-2', 'text-base');
      expect(screen.getByTestId('dialog-footer')).toHaveClass('mt-6', 'sm:justify-center');
    });
  });

  describe('Supabase Configuration', () => {
    it('should return null when Supabase is not enabled', () => {
      (supabaseConfig.isSupabaseEnabled as any).mockReturnValue(false);
  ...
  describe('Google Sign-In', () => {
    it('should call signInWithGoogle when button is clicked', async () => {
      const mockResponse = { provider: 'google' as const, url: 'https://auth.google.com/redirect' };
    ...
    it('should redirect to auth URL on successful sign-in', async () => {
      ...
      vi.mocked(api.signInWithGoogle).mockResolvedValue({ provider: 'google' as const, url: authUrl });
    ...
    it('should not redirect when no URL is returned', async () => {
      vi.mocked(api.signInWithGoogle).mockResolvedValue({ provider: 'google' as const, url: null as any });
  ...
  describe('Loading State', () => {
    it('should show loading text when signing in', async () => {
      vi.mocked(api.signInWithGoogle).mockImplementation(
        () => new Promise(resolve => setTimeout(() => resolve({ provider: 'google' as const, url: 'test' }), 100))
      );
    ...
    it('should reset loading state after successful sign-in', async () => {
      vi.mocked(api.signInWithGoogle).mockResolvedValue({ provider: 'google' as const, url: 'https://auth.test.com' });
  ...
  describe('Error Handling', () => {
    it('should display error message when sign-in fails', async () => {
      ...
      vi.mocked(api.signInWithGoogle).mockRejectedValue(new Error(errorMessage));
    ...
    it('should apply error styling to error message', async () => {
      vi.mocked(api.signInWithGoogle).mockRejectedValue(new Error('Test error'));
    ...
    it('should reset error state on new sign-in attempt', async () => {
      // First attempt fails
      vi.mocked(api.signInWithGoogle).mockRejectedValueOnce(new Error('First error'));
      ...
      vi.mocked(api.signInWithGoogle).mockResolvedValueOnce({ provider: 'google' as const, url: 'success' });
      await user.click(button);
  ...
  describe('Multiple Interactions', () => {
    it('should handle multiple rapid clicks', async () => {
      vi.mocked(api.signInWithGoogle).mockResolvedValue({ provider: 'google' as const, url: 'test' });
