...
describe('PopoverContentAuth', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(signInWithGoogle).mockResolvedValue({ provider: 'google', url: 'https://auth.google.com/oauth' });
  });
  ...
  describe('Initial render', () => {
    it('should render popover with banner image', () => {
      ...
      const bannerImage = screen.getByAltText(`calm paint generate by ${APP_NAME}`);
    ...
    it('should display login prompt text', () => {
      ...
      expect(screen.getByText('Login to try more features for free')).toBeInTheDocument();
      expect(screen.getByText('Add files, use more models, BYOK, and more.')).toBeInTheDocument();
    });
    ...
    it('should display Google logo in button', () => {
      ...
      expect(googleLogo).toHaveAttribute('src', 'https://www.google.com/favicon.ico');
    ...
    it('should have proper popover content attributes', () => {
      ...
      expect(popoverContent).toHaveClass('w-[300px]', 'overflow-hidden', 'rounded-xl', 'p-0');
    });
  });
  ...
  describe('Sign-in functionality', () => {
    ...
    it('should redirect to auth URL on successful sign-in', async () => {
      ...
      vi.mocked(signInWithGoogle).mockResolvedValue({ provider: 'google', url: authUrl });
    ...
    it('should handle sign-in without redirect URL', async () => {
      vi.mocked(signInWithGoogle).mockResolvedValue({} as { provider: 'google'; url: string });
  ...
  describe('Error handling', () => {
    ...
    it('should clear error on retry', async () => {
      vi.mocked(signInWithGoogle)
        ...
        .mockResolvedValueOnce({ provider: 'google', url: 'https://auth.google.com' });
  ...
  describe('Content layout', () => {
    it('should have proper content padding', () => {
      ...
      const contentContainer = screen.getByText('Login to try more features for free').closest('div');
    ...
    it('should have proper text styling', () => {
      ...
      expect(titleText).toHaveClass('mb-1', 'font-medium', 'text-base', 'text-primary');
      
      const descriptionText = screen.getByText('Add files, use more models, BYOK, and more.');
      expect(descriptionText).toHaveClass('mb-5', 'text-base', 'text-muted-foreground');
    });
  ...
  describe('Accessibility', () => {
    ...
    it('should have proper image alt text', () => {
      ...
      expect(screen.getByAltText(`calm paint generate by ${APP_NAME}`)).toBeInTheDocument();
