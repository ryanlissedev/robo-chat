...
console.log(
  'LANGSMITH_PROJECT:',
  process.env.LANGSMITH_PROJECT || '❌ Not set'
);
console.log(
  'LANGSMITH_ENDPOINT:',
...
console.log(
  'LANGSMITH_TRACING:',
  process.env.LANGSMITH_TRACING || '❌ Not set'
);
console.log(
  'LANGSMITH_TRACING_V2:',
  process.env.LANGSMITH_TRACING_V2 || '❌ Not set'
);
...
async function testLangSmithConnection() {
  ...
  try {
    ...
    console.log(
      `View in LangSmith: https://smith.langchain.com/o/${process.env.LANGSMITH_ORG || 'default'}/projects/p/${projectName}/runs/${runId}`
    );
