...
function testGatewayConfiguration() {
  ...
  console.log(
    `  Current gateway - enabled: ${config.enabled}, baseURL: ${config.baseURL}`
  );
  console.log(
    `  Environment - AI_GATEWAY_API_KEY: ${process.env.AI_GATEWAY_API_KEY ? '[SET]' : '[NOT SET]'}`
  );
  console.log(
    `  Environment - AI_GATEWAY_BASE_URL: ${process.env.AI_GATEWAY_BASE_URL || '[NOT SET]'}`
  );
...
async function runTests() {
  ...
  try {
    ...
    for (const testCase of PROVIDER_TESTS) {
      ...
      if (!testResults.providers[testCase.provider]) {
        testResults.providers[testCase.provider] = {
          passed: 0,
          failed: 0,
          total: 0,
        };
    ...
    // Calculate summary
    const allResults = [
      ...testResults.gateway.withoutGateway,
      ...testResults.gateway.withGateway,
    ];
    testResults.summary.total = allResults.length;
    testResults.summary.passed = allResults.filter(
      (r) => r.status === 'passed'
    ).length;
    testResults.summary.failed = allResults.filter(
      (r) => r.status === 'failed'
    ).length;
    testResults.summary.skipped = allResults.filter(
      (r) => r.status === 'skipped'
    ).length;
  } catch (error) {
  ...
  Object.entries(testResults.providers).forEach(([provider, stats]) => {
    ...
    console.log(
      `  ${provider}: ${stats.passed}/${stats.total} passed (${successRate}%)`
    );
  });

  console.log('\n🆕 OpenRouter Integration Status:');
  const openrouterResults = testResults.gateway.withoutGateway.filter(
    (r) => r.provider === 'openrouter'
  );
  const openrouterPassed = openrouterResults.filter(
    (r) => r.status === 'passed'
  ).length;
  console.log(`  OpenRouter models tested: ${openrouterResults.length}`);
  console.log(
    `  OpenRouter success rate: ${openrouterResults.length > 0 ? ((openrouterPassed / openrouterResults.length) * 100).toFixed(1) : 0}%`
  );
