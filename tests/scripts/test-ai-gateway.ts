...
async function runAllTests() {
  ...
  log(
    `OPENAI_API_KEY: ${OPENAI_API_KEY ? '✓ Found' : '✗ Missing'}`,
    OPENAI_API_KEY ? colors.green : colors.red
  );
  log(
    `VERCEL_AI_GATEWAY_API_KEY: ${GATEWAY_API_KEY ? '✓ Found' : '✗ Missing'}`,
    GATEWAY_API_KEY ? colors.green : colors.red
  );
  log(
    `ANTHROPIC_API_KEY: ${process.env.ANTHROPIC_API_KEY ? '✓ Found' : '✗ Missing'}`,
    process.env.ANTHROPIC_API_KEY ? colors.green : colors.red
  );
  ...
  if (!results.gatewayWithGatewayKey && !GATEWAY_API_KEY) {
    ...
    log(
      '3. Add to .env.local: VERCEL_AI_GATEWAY_API_KEY=your_key',
      colors.cyan
    );
  }
}
