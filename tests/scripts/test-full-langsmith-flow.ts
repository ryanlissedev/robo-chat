...
async function testFullFlow() {
  ...
  try {
    ...
    if (!runId) {
      console.log('   ℹ️  No run ID found. This may be expected if LangSmith tracing is disabled.');
    ...
    if (feedbackResponse.ok && feedbackResult.success) {
      ...
      if (feedbackResult.langsmith?.success) {
        ...
        console.log('   ⚠️  Feedback API worked but LangSmith submission failed:', feedbackResult.langsmith.error);
      }
