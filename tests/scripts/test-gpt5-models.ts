...
async function testChatCompletion(modelId: string, modelName: string) {
  ...
  try {
    ...
    console.log(`${colors.green}✓ Chat completions API successful${colors.reset}`);
    console.log(`Response: ${result.text.slice(0, 100)}...`);
    return true;
  } catch (error: any) {
    console.error(`${colors.yellow}⚠ Chat completions API failed (expected for GPT-5):${colors.reset}`);
...
async function main() {
  console.log(`${colors.cyan}=== GPT-5 Model Family Test Suite ===${colors.reset}`);
  ...
  for (const { id, name, isDefault } of testModels) {
    ...
    console.log(`${colors.cyan}Testing ${name}${isDefault ? ' (DEFAULT MODEL)' : ''}${colors.reset}`);
  ...
  const allResponsesPass = testModels.every(m => results[`${m.id}-responses`]);
  
  if (allResponsesPass) {
    console.log(`\n${colors.green}✅ All GPT-5 models are working with Responses API!${colors.reset}`);
    console.log(`\n${colors.cyan}Recommendations:${colors.reset}`);
    console.log(`• Use gpt-5-mini as the default model (best balance of speed/cost/capability)`);
    console.log(`• Use openai.responses() for GPT-5 models`);
    console.log(`• Configure textVerbosity and reasoningSummary for optimal results`);
  } else {
