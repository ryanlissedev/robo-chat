...
async function testRealRunId() {
  ...
  try {
    ...
    chatResponse.headers.forEach((value, key) => {
      if (
        key.toLowerCase().includes('langsmith') ||
    ...
    if (finalRunId) {
      ...
      console.log(
        '  1. LangSmith tracing is not properly configured for AI SDK responses'
      );
      console.log(
        '  2. The AI SDK is not returning run IDs in headers or response data'
      );
      console.log(
        '  3. Run IDs are generated but not exposed in the streaming response'
      );
      ...
      const mockFeedbackResponse = await fetch(`${BaseUrl}/api/feedback`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.log(
        ...
        JSON.stringify(mockFeedbackResult, null, 2)
      );
...
testRealRunId().then((success) => {
  console.log(`\n${'='.repeat(60)}`);
  console.log(
    success ? '✅ Real run ID test completed!' : '❌ Real run ID test failed'
  );
