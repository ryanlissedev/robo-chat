...
async function createRealtimeAnswer(
  ...
  const realtimeConfig = {
    ...
    tool_choice: 'auto',
    temperature:
      typeof safeConfig.temperature === 'number' ? safeConfig.temperature : 0.7,
...
function generateInstructions(config: VoiceSessionConfig): string {
  ...
  const personalityMode =
    config && typeof config.personalityMode === 'string'
      ? (config.personalityMode as keyof typeof personalityInstructions)
      : 'safety-focused';

  const selectedPersonality =
    personalityInstructions[personalityMode] ||
    personalityInstructions['safety-focused'];

  // Safely check safety protocols
  const safetyProtocols =
    config && config.safetyProtocols !== false
      ? `
...
export async function GET() {
  try {
    ...
    const offers = Array.from(pendingOffers.entries()).map(
      ([sessionId, offer]) => ({
        sessionId,
