...
export function VectorStoreManager({ userId }: VectorStoreManagerProps) {
  ...
  const loadVectorStores = useCallback(async () => {
    ...
    try {
      ...
      const stores = response.data.map((store: OpenAI.VectorStore) => ({
        id: store.id,
  ...
  const loadStoreFiles = useCallback(async (storeId: string) => {
    ...
    try {
      ...
      const response = await openai.vectorStores.files.list(storeId);
      const files = response.data.map((file: OpenAI.VectorStoreFile) => ({
        id: file.id,
  ...
  const deleteVectorStore = async (storeId: string) => {
    ...
    try {
      ...
      await openai.vectorStores.delete(storeId);
  ...
  const onDrop = useCallback(
      ...
      try {
        ...
        for (const file of acceptedFiles) {
          try {
            ...
            await openai.vectorStores.files.create(selectedStore.id, {
    ...
    [selectedStore, userId, supabase, loadStoreFiles]
  );
