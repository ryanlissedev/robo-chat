  ...
  const createPersistedGroups = useCallback(() => {
    ...
    for (let i = 0; i < persistedMessages.length; i++) {
      ...
      if (message.role === "user") {
        const groupKey = message.parts?.filter(p => p.type === 'text').map(p => (p as { text: string }).text).join(' ') || `message-${i}`
        if (!groups[groupKey]) {
          groups[groupKey] = {
            userMessage: message,
            assistantMessages: [],
        ...
        if (associatedUserMessage) {
          const groupKey = associatedUserMessage.parts?.filter(p => p.type === 'text').map(p => (p as { text: string }).text).join(' ') || `user-message-${j}`
          if (!groups[groupKey]) {
            groups[groupKey] = {
    ...
    modelChats.forEach((chat) => {
      for (let i = 0; i < chat.messages.length; i += 2) {
        ...
        if (userMsg?.role === "user") {
          const groupKey = userMsg.parts?.filter(p => p.type === 'text').map(p => (p as { text: string }).text).join(' ') || `live-message-${i}`
          ...
          if (assistantMsg?.role === "assistant") {
            ...
            chat.isLoading &&
            (userMsg.parts?.filter(p => p.type === 'text').map(p => (p as { text: string }).text).join(' ') || '') === prompt &&
