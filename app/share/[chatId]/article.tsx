...
// Updated to UIMessage naming in AI SDK v5
...
// Narrow unknown/DB parts shape to UI parts safely
function toUIParts(input: unknown): MessageAISDK['parts'] {
  if (!Array.isArray(input)) return [];
  // Keep only objects with a string `type` field; other shapes are ignored
  return input.filter((p): p is { type: string } => {
    return !!p && typeof p === 'object' && 'type' in p && typeof (p as Record<string, unknown>).type === 'string';
  }) as MessageAISDK['parts'];
}
      ...
      <div className="mx-auto max-w-3xl px-4 py-12 md:py-24">
        ...
        <div className="mt-20 w-full">
          {messages.map((message) => {
            const parts = toUIParts(message?.parts);
            ...
            return (
              <div key={message.id}>
                ...
                >
                  ...
                  >
                    {
                      // Prefer legacy content if present, else first text part
                      message.content ||
                        (parts.find((p) => p.type === 'text' && 'text' in p) as
